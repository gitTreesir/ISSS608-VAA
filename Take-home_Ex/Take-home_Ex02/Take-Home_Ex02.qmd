...
V(mc2_graph)$eigen_score <- eigen_centrality
...
1.  In order not to overclutter the network graph, all edges with weight \<= 50 will be filtered out and not included.

2.  For the ease of selecting a particular entity, the dropdown list has been sorted in ascending order by using `arrange()`.

3.  In addition, directional arrows have also been included on the edges, to help identify the nodes that are highly connected to other nodes in the network.

4.  Hover action is also enabled for the graph so that the user can hover the mouse pointer over the graph to look at the possible different 'groups' of connectivity.
...
From the above network graph, we noticed that some of the nodes in the middle seemed to be denser compared to the rest, indicating that these nodes probably have more connections to the other nodes. With the help of the hover function, we were able to identify the ids of these nodes. However, this requires the user to maneuver around the graph with great efforts.
...
With that information, we will study these 4 entities in greater details.
:::
...
nodes_df_eigen_top4 <- nodes_df %>%
  ...
  select(id, label, eigen_score, eigen_bin, deg_bin, 
         in_bin, out_bin, bet_bin, close_bin) %>%
  arrange(desc(eigen_score))

top_4_eigen <- head(nodes_df_eigen_top4, 4)

datatable(top_4_eigen, 
...
edges_df_eigen <- edges_df %>%
  filter(from %in% top_4_eigen$id |
         to %in% top_4_eigen$id) %>%
  filter(weight > 50)
...
::: {.callout-note title="Observations"}
Based on the information presented in the DataTable, we observed that these 4 nodes also have very high number of in-degree centrality index, indicating that they are  likely to be the fishery wharves/central collection centers within the network. 
:::
...
The graph above revealed that there are **18** nodes with high degree centrality index of \>= 300. This means that these nodes have significantly high number of direct connections to other nodes within the network. 
:::
...
nodes_df_deg <- nodes_df %>%
  ...
  select(id, label, degree_score, deg_bin, eigen_bin,
         close_bin, bet_bin, in_degree, in_bin, 
         out_degree, out_bin) %>%  
  arrange(desc(degree_score))

datatable(nodes_df_deg, 
          colnames = c("ID", "Entity", "Degree Centrality Index", 
                       "Degree Centrality Category",                        
                       "Eigenvector Centrality Category", 
                       "Closeness Centrality Category",
                       "Betweenness Centrality Category",
                       ...
                       "Out-Degree Centrality Cateogy"),
          options = list(
            columnDefs = list(list(className = 'dt-center', targets = 11))))
```
...
nodes_df_out <- nodes_df %>%
  ...
  select(id, label, out_degree, out_bin, eigen_bin, 
         deg_bin, close_bin, bet_bin, in_bin) %>%
