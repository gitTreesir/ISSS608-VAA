...
# group the finjournal data by participantId and get the transaction count for each participant. We will also arrange the transaction count in ascending order.
...
For the graph below, users can select/unselect the different education levels to show on the graph by clicking on the legend on the right side of the chart. The actual total savings and income figures will also be displayed as hovering text when the cursor moves over the points.
...
bp <- expanded_merged_data %>%
  ...
  layout(title = "Monthly Income vs % of Monthly Income Spent\n",
         xaxis = list(title = "\nMonthly Income"),
...
```
[**Explanation**]{.underline}
From the graph, it appeared that there were some overspendings among the residents across the months. These residents typically belong to the lower education groups.
